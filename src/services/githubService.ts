...
const searchRepos = (searchText: string, config?: any): Promise<AxiosResponse<SearchReposResults>> => {
...
function searchUsers(searchText: string): Promise<AxiosResponse<SearchUsersResults>> {
    const encondedQuery: string = encodeURIComponent(`${searchText} user:${searchText}`);
